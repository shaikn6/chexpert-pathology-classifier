...
IMAGE_NAME := chexpert-pathology-classifier
SRC := src
PYTHON := python3
...
# ─── Setup ───────────────────────────────────────────────────────────────────
install: ## Install dependencies (CPU-only PyTorch for dev)
...
# ─── Testing ──────────────────────────────────────────────────────────────────
test: ## Run test suite with coverage (DenseNet/CheXpert unit tests)
	PYTHONPATH=. pytest tests/ -v --cov=$(SRC) --cov-report=term-missing --cov-report=xml --cov-fail-under=80

test-fast: ## Run tests without coverage (faster)
	PYTHONPATH=. pytest tests/ -v -x

test-parallel: ## Run tests in parallel
	PYTHONPATH=. pytest tests/ -v -n auto --cov=$(SRC)
...
clean: ## Remove build artifacts and cache
	...
	rm -f bandit-report.json pip-audit-report.json coverage.xml mlflow.db
